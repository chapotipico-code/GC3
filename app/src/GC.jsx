...
import {
  ...
  Menu, MessageSquare, AlertCircle, Sun, Moon, ShieldCheck, Radio, RefreshCw
} from "lucide-react";
...
const computeRg = (st) => (st.mode === "gun" ? { from: st.day, to: st.day, label: trDate(st.day) } : st.mode === "aralik" ? { from: st.from || st.day, to: st.to || st.day, label: `${trDate(st.from || st.day)} – ${trDate(st.to || st.day)}` } : rangeFor(st.mode));
...
function DateFilter({ st, setSt }) { const presets = [["bugun", "Bugün"], ["dun", "Dün"], ["son7", "Son 7 Gün"], ["buay", "Bu Ay"], ["gecenay", "Geçen Ay"], ["tumu", "Tümü"]]; return (<div className="flex gap-2 flex-wrap items-center">{presets.map(([k, l]) => <Chip key={k} on={st.mode === k} onClick={() => setSt({ ...st, mode: k })}>{l}</Chip>)}<input type="date" className={inputCls} style={{ width: 150 }} value={st.day} onChange={(e) => setSt({ ...st, mode: "gun", day: e.target.value })} /><div className={"flex gap-1 items-center rounded-lg px-2 py-1 " + (st.mode === "aralik" ? "bg-blue-600/20 border border-blue-500/50" : "bg-slate-950 border border-slate-800")}><span className="text-xs text-slate-500">Aralık:</span><input type="date" className={inputCls} style={{ width: 140 }} value={st.from || st.day} onChange={(e) => setSt({ ...st, mode: "aralik", from: e.target.value, to: st.to || st.day })} /><span className="text-slate-500">–</span><input type="date" className={inputCls} style={{ width: 140 }} value={st.to || st.day} onChange={(e) => setSt({ ...st, mode: "aralik", from: st.from || st.day, to: e.target.value })} /></div></div>); }
...
export default function GC() {
  ...
  // Karşılaştırma metni: anahtar sırasından bağımsız (gereksiz kayıt/çakışma olmasın)
  const STATE_KEYS = ["firms", "tx", "wd", "shares", "shareTx", "transfers", "adjusts", "expenses", "botToken", "notify", "contacts", "schedule", "templates", "auditLog", "trash", "ipAllow", "autoBackup", "bannedIps"];
  const canonStr = (o) => JSON.stringify(STATE_KEYS.map((k) => (o ? o[k] : undefined)));

  // Kayıtlar sıraya girer: aynı anda iki PUT gitmez (sürüm çakışması/hata önlenir)
  const savingRef = useRef(false);
  const dirtyRef = useRef(false);
  const gatherRef = useRef(gather); gatherRef.current = gather;
  const applyRef = useRef(applyData); applyRef.current = applyData;
  const mergeRef = useRef(mergeData); mergeRef.current = mergeData;
  const saveNowRef = useRef(null);
  const appendRemote = async (payload) => {
    try { const r = await api("/api/append", { method: "POST", body: JSON.stringify(payload) }); if (r && r.version) dataRef.current.version = r.version; }
    catch (e) { dirtyRef.current = true; if (saveNowRef.current) setTimeout(() => saveNowRef.current(), 400); }
  };
  const removeRemote = async (payload) => { try { const r = await api("/api/remove", { method: "POST", body: JSON.stringify(payload) }); if (r && r.version) dataRef.current.version = r.version; } catch (e) { dirtyRef.current = true; if (saveNowRef.current) setTimeout(() => saveNowRef.current(), 400); } };
  const patchRemote = async (ops, audit) => { try { const r = await api("/api/patch", { method: "POST", body: JSON.stringify({ ops, audit }) }); if (r && r.version) dataRef.current.version = r.version; } catch (e) { dirtyRef.current = true; if (saveNowRef.current) setTimeout(() => saveNowRef.current(), 400); } };
  const saveNow = async () => {
    if (savingRef.current) { dirtyRef.current = true; return; }
    savingRef.current = true;
    try {
      const d = gatherRef.current();
      const str = canonStr(d);
      ...
      try {
        ...
        if (e.status === 409 && e.body) {
          // Çakışma: yereli silme — birleştir, ekleneni koru, tekrar kaydet
          const merged = mergeRef.current(e.body.data, gatherRef.current());
          ...
          try {
            ...
            dataRef.current.version = s2.version; dataRef.current.last = canonStr(merged);
          } catch (e2) {
            dataRef.current.version = (e2.body && e2.body.version) || dataRef.current.version;
            dirtyRef.current = true; // sonraki turda tekrar denenecek
          }
        } else {
          dirtyRef.current = true; // ağ/sunucu hatası → sessizce tekrar dene
        }
      }
    } finally {
      savingRef.current = false;
      if (dirtyRef.current) { dirtyRef.current = false; setTimeout(() => saveNowRef.current && saveNowRef.current(), 600); }
    }
  };
  ...
  const loadState = async () => {
    ...
    if (r.data && Object.keys(r.data).length) {
      applyData(r.data);
      dataRef.current.last = canonStr(r.data);
    } else {
      const d = gather();
      try { const s = await api("/api/state", { method: "PUT", body: JSON.stringify({ data: d, baseVersion: r.version }) }); dataRef.current.version = s.version; dataRef.current.last = canonStr(d); } catch (e) {}
    }
    setHydrated(true);
  };
  ...
  // Yerel değişiklikleri sunucuya kaydet (debounce + sıraya alma)
  useEffect(() => {
    if (!hydrated || !auth) return;
    if (canonStr(gather()) === dataRef.current.last) return;
    const t = setTimeout(() => { saveNowRef.current && saveNowRef.current(); }, 800);
  ...
  useEffect(() => {
    ...
    const iv = setInterval(async () => {
      try {
        // Kayıt sürüyorsa ya da kaydedilmemiş değişiklik varsa atla (ezme önlenir)
        if (savingRef.current || dirtyRef.current) return;
        if (canonStr(gatherRef.current()) !== dataRef.current.last) return;
        const r = await api("/api/state");
        if (r.version !== dataRef.current.version) {
          const str = canonStr(r.data);
          if (str !== dataRef.current.last) { applyRef.current(r.data); dataRef.current.last = str; }
          dataRef.current.version = r.version;
        }
      } catch (e) {}
    }, 5000);
  ...
  const errSentRef = useRef(new Map());
  const pushError = (message, stack, ctx) => {
    setErrorLog((l) => [{ id: nid(), at: new Date(), message, stack: stack || "" }, ...l].slice(0, 300));
    try {
      // Aynı hata 60 sn içinde tekrar gelirse sunucuya tekrar yazma (döngü/spam koruması)
      const key = String(message) + "|" + String(stack || "").split("\n")[0];
      const now = Date.now();
      const seen = errSentRef.current.get(key);
      if (seen && now - seen < 60000) return;
      errSentRef.current.set(key, now);
      if (errSentRef.current.size > 100) errSentRef.current.clear();
      if (TOKEN) api("/api/errors", { method: "POST", body: JSON.stringify({ message: String(message), stack: String(stack || ""), ctx: ctx || "panel" }) }).catch(() => {});
    } catch (e) {}
  };
  const pushErrorRef = useRef(null); pushErrorRef.current = pushError;
  // Paneldeki TÜM hataları yakala (React dışı JS hataları ve yakalanmamış promise'ler dahil)
  useEffect(() => {
    const onErr = (e) => { if (!e || !e.message) return; try { pushErrorRef.current(e.message, (e.error && e.error.stack) || (e.filename ? `${e.filename}:${e.lineno}:${e.colno}` : ""), "js"); } catch (x) {} };
    const onRej = (e) => { try { const r = e && e.reason; pushErrorRef.current((r && r.message) || String(r), (r && r.stack) || "", "promise"); } catch (x) {} };
  ...
  const addDeposit = ({ firmId, amount, date, note }) => { const f = firms.find((x) => x.id === firmId); if (!f) return; date = date || todayIso(); const gross = Number(amount), commission = r2(gross * f.rate / 100), net = r2(gross - commission), id = nid(); const txObj = { id, firmId, gross, rate: f.rate, commission, net, date, note: note || "", ord: nextOrd() }; const dist = distribute(commission, f.name, date, id, "dep"); const aud = { id: nid(), at: new Date().toISOString(), user: auth ? auth.username : "-", action: "Yatırım eklendi", detail: `${f.name} · ${fmt(gross)}` }; setTx((t) => [txObj, ...t]); setShareTx((st) => [...dist, ...st]); setAuditLog((l) => [aud, ...l].slice(0, 300)); appendRemote({ tx: txObj, shareTx: dist, audit: aud }); };
  const editDeposit = (id, { firmId, amount, date, note }) => { const f = firms.find((x) => x.id === firmId); if (!f) return; date = date || todayIso(); const old = tx.find((x) => x.id === id); const gross = Number(amount), commission = r2(gross * f.rate / 100), net = r2(gross - commission); setTx((t) => t.map((x) => (x.id === id ? { ...x, firmId, gross, rate: f.rate, commission, net, date, note } : x))); const dist = distribute(commission, f.name, date, id, "dep"); setShareTx((st) => [...dist, ...st.filter((p) => p.sourceId !== id)]); pushAudit("Yatırım düzenlendi", diffDetail([["Firma", firms.find((x) => x.id === old.firmId)?.name, f.name], ["Tutar", fmt(old.gross), fmt(gross)], ["Tarih", trDate(old.date), trDate(date)]])); patchRemote([{ op: "update", arr: "tx", item: { id, firmId, gross, rate: f.rate, commission, net, date, note } }, { op: "removeBy", arr: "shareTx", field: "sourceId", value: id }, ...dist.map((e) => ({ op: "add", arr: "shareTx", item: e }))]); };
  const delDeposit = (id) => { const t = tx.find((x) => x.id === id); if (!t) return; const sx = shareTx.filter((p) => p.sourceId === id); const trashEntry = { id: nid(), type: "tx", label: `Yatırım · ${firms.find((f) => f.id === t.firmId)?.name} · ${fmt(t.gross)} · ${trDate(t.date)}`, at: new Date().toISOString(), by: auth.username, payload: { tx: t, sx } }; const aud = { id: nid(), at: new Date().toISOString(), user: auth ? auth.username : "-", action: "Yatırım çöpe taşındı", detail: "" }; setTx((a) => a.filter((x) => x.id !== id)); setShareTx((a) => a.filter((p) => p.sourceId !== id)); setTrash((tr) => [trashEntry, ...tr]); setAuditLog((l) => [aud, ...l].slice(0, 300)); removeRemote({ kind: "tx", id, trash: trashEntry, audit: aud }); };

  const addWithdrawal = ({ firmId, amount, date, note, noComm }) => { const f = firms.find((x) => x.id === firmId); if (!f) return; date = date || todayIso(); const net = Number(amount), wr = noComm ? 0 : f.wrate, commission = r2(net * wr / 100), gross = r2(net + commission), id = nid(); const wdObj = { id, firmId, net, rate: wr, commission, gross, date, note: note || "", noComm: !!noComm, ord: nextOrd() }; const dist = distribute(commission, f.name, date, id, "wd"); const aud = { id: nid(), at: new Date().toISOString(), user: auth ? auth.username : "-", action: "Çekim eklendi", detail: `${f.name} · ${fmt(net)}${noComm ? " · komisyonsuz" : ""}` }; setWd((w) => [wdObj, ...w]); setShareTx((st) => [...dist, ...st]); setAuditLog((l) => [aud, ...l].slice(0, 300)); appendRemote({ wd: wdObj, shareTx: dist, audit: aud }); };
  const editWithdrawal = (id, { firmId, amount, date, note, noComm }) => { const f = firms.find((x) => x.id === firmId); if (!f) return; date = date || todayIso(); const old = wd.find((x) => x.id === id); const net = Number(amount), wr = noComm ? 0 : f.wrate, commission = r2(net * wr / 100), gross = r2(net + commission); setWd((w) => w.map((x) => (x.id === id ? { ...x, firmId, net, rate: wr, commission, gross, date, note, noComm: !!noComm } : x))); const dist = distribute(commission, f.name, date, id, "wd"); setShareTx((st) => [...dist, ...st.filter((p) => p.sourceId !== id)]); pushAudit("Çekim düzenlendi", diffDetail([["Firma", firms.find((x) => x.id === old.firmId)?.name, f.name], ["Net", fmt(old.net), fmt(net)], ["Komisyon", old.noComm ? "yok" : "var", noComm ? "yok" : "var"], ["Tarih", trDate(old.date), trDate(date)]])); patchRemote([{ op: "update", arr: "wd", item: { id, firmId, net, rate: wr, commission, gross, date, note, noComm: !!noComm } }, { op: "removeBy", arr: "shareTx", field: "sourceId", value: id }, ...dist.map((e) => ({ op: "add", arr: "shareTx", item: e }))]); };
  const delWithdrawal = (id) => { const w = wd.find((x) => x.id === id); if (!w) return; const sx = shareTx.filter((p) => p.sourceId === id); const trashEntry = { id: nid(), type: "wd", label: `Çekim · ${firms.find((f) => f.id === w.firmId)?.name} · ${fmt(w.net)} · ${trDate(w.date)}`, at: new Date().toISOString(), by: auth.username, payload: { wd: w, sx } }; const aud = { id: nid(), at: new Date().toISOString(), user: auth ? auth.username : "-", action: "Çekim çöpe taşındı", detail: "" }; setWd((a) => a.filter((x) => x.id !== id)); setShareTx((a) => a.filter((p) => p.sourceId !== id)); setTrash((tr) => [trashEntry, ...tr]); setAuditLog((l) => [aud, ...l].slice(0, 300)); removeRemote({ kind: "wd", id, trash: trashEntry, audit: aud }); };

  const addTransfer = ({ fromFirmId, toFirmId, amount, date, note }) => { const item = { id: nid(), fromFirmId, toFirmId, amount: Number(amount), date, note: note || "", ord: nextOrd() }; setTransfers((t) => [item, ...t]); pushAudit("Virman", `${firms.find((f) => f.id === fromFirmId)?.name} → ${firms.find((f) => f.id === toFirmId)?.name} · ${fmt(amount)}`); patchRemote([{ op: "add", arr: "transfers", item }]); };
  const delTransfer = (id) => { const t = transfers.find((x) => x.id === id); if (!t) return; const trashEntry = { id: nid(), type: "transfer", label: `Virman · ${firms.find((f) => f.id === t.fromFirmId)?.name} → ${firms.find((f) => f.id === t.toFirmId)?.name} · ${fmt(t.amount)}`, at: new Date().toISOString(), by: auth.username, payload: { transfer: t } }; setTransfers((a) => a.filter((x) => x.id !== id)); setTrash((tr) => [trashEntry, ...tr]); pushAudit("Virman çöpe taşındı", ""); patchRemote([{ op: "remove", arr: "transfers", id }, { op: "add", arr: "trash", item: trashEntry }]); };

  const restoreTrash = (id) => { const it = trash.find((x) => x.id === id); if (!it) return; const ops = []; if (it.type === "tx") { setTx((a) => [it.payload.tx, ...a]); setShareTx((a) => [...it.payload.sx, ...a]); ops.push({ op: "add", arr: "tx", item: it.payload.tx }); (it.payload.sx || []).forEach((e) => ops.push({ op: "add", arr: "shareTx", item: e })); } else if (it.type === "wd") { setWd((a) => [it.payload.wd, ...a]); setShareTx((a) => [...it.payload.sx, ...a]); ops.push({ op: "add", arr: "wd", item: it.payload.wd }); (it.payload.sx || []).forEach((e) => ops.push({ op: "add", arr: "shareTx", item: e })); } else if (it.type === "transfer") { setTransfers((a) => [it.payload.transfer, ...a]); ops.push({ op: "add", arr: "transfers", item: it.payload.transfer }); } setTrash((tr) => tr.filter((x) => x.id !== id)); ops.push({ op: "remove", arr: "trash", id }); pushAudit("Çöpten geri yüklendi", it.label); patchRemote(ops); };
  const purgeTrash = (id) => { setTrash((tr) => tr.filter((x) => x.id !== id)); patchRemote([{ op: "remove", arr: "trash", id }]); };

  const addAdjust = ({ firmId, amount, date, note }) => { const item = { id: nid(), firmId, amount: Number(amount), date, note: note || "", ord: nextOrd() }; setAdjusts((a) => [item, ...a]); pushAudit("Kasa düzeltme (komisyonsuz)", `${firms.find((x) => x.id === firmId)?.name} · ${Number(amount) >= 0 ? "+" : ""}${fmt(amount)}`); patchRemote([{ op: "add", arr: "adjusts", item }]); };

  const addShareTx = ({ shareId, kind, amount, date, desc }) => { const signed = kind === "withdraw" ? -Math.abs(amount) : Math.abs(amount); const item = { id: nid(), shareId, type: kind, amount: signed, date, desc: desc || (kind === "withdraw" ? "Çekim" : "Manuel ekleme"), ord: nextOrd() }; setShareTx((st) => [item, ...st]); patchRemote([{ op: "add", arr: "shareTx", item }]); };
  const editShareTx = (id, { amount, date, desc }) => { const old = shareTx.find((x) => x.id === id); if (!old) return; const a = old.type === "withdraw" ? -Math.abs(amount) : Math.abs(amount); setShareTx((st) => st.map((x) => x.id === id ? { ...x, amount: a, date, desc } : x)); pushAudit("Kar payı hareketi düzenlendi", diffDetail([["Tutar", fmt(Math.abs(old.amount)), fmt(Math.abs(amount))], ["Tarih", trDate(old.date), trDate(date)], ["Açıklama", old.desc, desc]])); patchRemote([{ op: "update", arr: "shareTx", item: { id, amount: a, date, desc } }]); };
  const delShareTx = (id) => { setShareTx((st) => st.filter((x) => x.id !== id)); patchRemote([{ op: "remove", arr: "shareTx", id }]); };
  ...
  const addExpense = (d) => { const id = nid(); const item = { id, title: d.title, amount: Number(d.amount), category: d.category, date: d.date, note: d.note || "", ord: nextOrd() }; const dist = distributeExpense(Number(d.amount), id, d.date); setExpenses((e) => [item, ...e]); setShareTx((st) => [...dist, ...st]); pushAudit("Gider eklendi", `${d.title} · ${fmt(d.amount)}`); patchRemote([{ op: "add", arr: "expenses", item }, ...dist.map((e) => ({ op: "add", arr: "shareTx", item: e }))]); };
  const editExpense = (id, d) => { const old = expenses.find((x) => x.id === id); const dist = distributeExpense(Number(d.amount), id, d.date); setExpenses((e) => e.map((x) => x.id === id ? { ...x, title: d.title, amount: Number(d.amount), category: d.category, date: d.date, note: d.note } : x)); setShareTx((st) => [...dist, ...st.filter((p) => p.sourceId !== id)]); pushAudit("Gider düzenlendi", diffDetail([["Başlık", old.title, d.title], ["Tutar", fmt(old.amount), fmt(d.amount)], ["Kategori", old.category, d.category], ["Tarih", trDate(old.date), trDate(d.date)]])); patchRemote([{ op: "update", arr: "expenses", item: { id, title: d.title, amount: Number(d.amount), category: d.category, date: d.date, note: d.note } }, { op: "removeBy", arr: "shareTx", field: "sourceId", value: id }, ...dist.map((e) => ({ op: "add", arr: "shareTx", item: e }))]); };
  const delExpense = (id) => { setExpenses((e) => e.filter((x) => x.id !== id)); setShareTx((a) => a.filter((p) => p.sourceId !== id)); pushAudit("Gider silindi", ""); patchRemote([{ op: "remove", arr: "expenses", id }, { op: "removeBy", arr: "shareTx", field: "sourceId", value: id }]); };
  ...
  const ALL = [
    ...
    { id: "denetim", label: "Denetim", icon: ShieldCheck, roles: FULL },
    { id: "telegram", label: "Telegram", icon: Send, roles: FULL },
    ...
    <Shell auth={auth} tabs={tabs} onLogout={logout} theme={theme} onToggleTheme={toggleTheme} onSecurity={() => setSecurity(true)}>
      {(tab) => (
        <ErrorBoundary onError={pushError}>
          ...
          {tab === "islem" && <Islem auth={auth} firms={firms} tx={tx} firmBalance={firmBalance} addDeposit={addDeposit} editDeposit={editDeposit} delDeposit={delDeposit} setConfirm={setConfirm} />}
          ...
          {tab === "rapor" && <Rapor firms={firms} tx={tx} wd={wd} shares={shares} shareTx={shareTx} expenses={expenses} adjusts={adjusts} transfers={transfers} firmBalance={firmBalance} />}
          {tab === "denetim" && <Denetim firms={firms} tx={tx} wd={wd} transfers={transfers} adjusts={adjusts} auditLog={auditLog} firmBalance={firmBalance} addDeposit={addDeposit} editDeposit={editDeposit} delDeposit={delDeposit} pushAudit={pushAudit} setConfirm={setConfirm} />}
          {tab === "telegram" && <TgYonetim firms={firms} />}
...
function Shell({ auth, tabs, onLogout, theme, onToggleTheme, onSecurity, children }) {
  ...
  const SideInner = (
    <div className="w-60 shrink-0 bg-slate-900 border-r border-slate-800 p-3 flex flex-col h-full">
      <div className="flex items-center gap-2 px-2 py-3"><div className="w-9 h-9 rounded-xl bg-blue-600 flex items-center justify-center font-black text-white">GC</div><div><div className="font-bold leading-none">GC Sistem</div><Badge tone="green">v2.0</Badge></div></div>
...
/* ============ ortak islem modali ============ */
function TxEditModal({ title, firms, init, isWd, masked, firmBalance, onClose, onSave }) {
  ...
  return (
    <Modal title={title} onClose={onClose} footer={<><button className={btnG} onClick={onClose}>İptal</button><button className={btnP} onClick={() => { if (!firmId || !v) return; onSave({ firmId: Number(firmId), amount: v, date: date || todayIso(), note, noComm: isWd ? noComm : undefined }); }}>Kaydet</button></>}>
...
/* ============ Islem Ekleme ============ */
function Islem({ auth, firms, tx, firmBalance, addDeposit, editDeposit, delDeposit, setConfirm }) {
  ...
  return (
    <div>
      ...
      {!masked && lastResult && (<div className="mb-4 rounded-xl border border-emerald-500/40 bg-emerald-500/10 px-4 py-3 text-sm flex items-center justify-between flex-wrap gap-2"><span><b>{lastResult.name}</b> · işlem eklendi — Önceki bakiye: <b className="text-slate-300">{fmt(lastResult.before)}</b> &rarr; İşlem sonrası: <b className="text-emerald-300">{fmt(lastResult.after)}</b> <span className="text-emerald-400">(+{fmt(lastResult.delta)})</span></span><button className="text-emerald-300 hover:text-emerald-100" onClick={() => setLastResult(null)}><X size={15} /></button></div>)}
      ...
      {modal === "new" && <TxEditModal title="Yatırım Ekle" firms={firms} masked={masked} firmBalance={firmBalance} init={{ firmId: firmFilter ? Number(firmFilter) : undefined, date: st.mode === "gun" ? st.day : todayIso() }} isWd={false} onClose={() => setModal(null)} onSave={(d) => { const f = firms.find((x) => x.id === d.firmId); const before = firmBalance(d.firmId); const net = r2(d.amount - r2(d.amount * (f?.rate || 0) / 100)); addDeposit(d); if (!masked) setLastResult({ name: f?.name || "", before, after: r2(before + net), delta: net }); const dt = d.date || todayIso(); setSt(dt === todayIso() ? { mode: "bugun", day: todayIso() } : { mode: "gun", day: dt }); setQ(""); setPage(1); setModal(null); }} />}
      {modal && modal !== "new" && <TxEditModal title="Yatırım Düzenle" firms={firms} firmBalance={firmBalance} masked={masked} masked={masked} init={{ firmId: modal.firmId, amount: modal.gross, date: modal.date, note: modal.note }} isWd={false} onClose={() => setModal(null)} onSave={(d) => { editDeposit(modal.id, d); setModal(null); }} />}
    </div>
  );
}
...
function Cekim({ firms, wd, firmBalance, addWithdrawal, editWithdrawal, delWithdrawal, setConfirm }) {
  ...
  return (
    <div>
      ...
      {modal === "new" && <TxEditModal title="Çekim Ekle" firms={firms} firmBalance={firmBalance} init={{ firmId: firmFilter ? Number(firmFilter) : undefined, date: st.mode === "gun" ? st.day : todayIso() }} isWd onClose={() => setModal(null)} onSave={(d) => { addWithdrawal(d); const dt = d.date || todayIso(); setSt(dt === todayIso() ? { mode: "bugun", day: todayIso() } : { mode: "gun", day: dt }); setQ(""); setPage(1); setModal(null); }} />}
      {modal && modal !== "new" && <TxEditModal title="Çekim Düzenle" firms={firms} firmBalance={firmBalance} init={{ firmId: modal.firmId, amount: modal.net, date: modal.date, note: modal.note, noComm: modal.noComm }} isWd onClose={() => setModal(null)} onSave={(d) => { editWithdrawal(modal.id, d); setModal(null); }} />}
    </div>
  );
}
...
function Komisyon({ shares, setShares, shareTx, tx, wd, shareBalance, addShareTx, editShareTx, delShareTx, setConfirm }) {
  ...
  const [lfil, setLfil] = useState("all");
  ...
  const walletTx = wallet ? shareTx.filter((p) => p.shareId === wallet).sort((a, b) => b.ord - a.ord) : [];
  const ledger = lfil === "all" ? walletTx : walletTx.filter((p) => p.type === lfil);
  const lIn = ledger.filter((p) => p.amount >= 0).reduce((a, p) => a + p.amount, 0);
  const lOut = ledger.filter((p) => p.amount < 0).reduce((a, p) => a + p.amount, 0);
  const LTABS = [["all", "Tümü"], ["commission", "Komisyon"], ["add", "Kasa Ekleme"], ["withdraw", "Çekim"], ["expense", "Gider Payı"]];
  const wShare = wallet ? shares.find((s) => s.id === wallet) : null;
  const PT = { commission: ["Komisyon", "blue"], add: ["Kasa Ekleme", "green"], withdraw: ["Çekim", "red"], expense: ["Gider Payı", "amber"] };
  return (
    <div>
      ...
      {wShare && (
        <Modal title={wShare.name + " — Kasa & Defter"} onClose={() => setWallet(null)} wide footer={<button className={btnG} onClick={() => setWallet(null)}>Kapat</button>}>
          ...
          <div className="border-t border-slate-800 pt-3 mb-3">
            <div className="flex gap-2 mb-3 flex-wrap gc-btnrow">{LTABS.map(([k, l]) => { const n = k === "all" ? walletTx.length : walletTx.filter((p) => p.type === k).length; return <Chip key={k} on={lfil === k} onClick={() => setLfil(k)}>{l} <span className="opacity-60">({n})</span></Chip>; })}</div>
            <div className="flex gap-4 mb-2 text-xs flex-wrap"><span className="text-slate-500">Kayıt: <b className="text-slate-300">{ledger.length}</b></span><span className="text-slate-500">Giren: <b className="text-emerald-400">{fmt(lIn)}</b></span><span className="text-slate-500">Çıkan: <b className="text-red-400">{fmt(lOut)}</b></span><span className="text-slate-500">Net: <b className={lIn + lOut >= 0 ? "text-emerald-400" : "text-red-400"}>{fmt(lIn + lOut)}</b></span></div>
          </div>
...
function Bildirim({ firms, shares, tx, wd, firmBalance, shareTx, botToken, setBotToken, notify, setNotify, contacts, setContacts, schedule, setSchedule, templates, setTemplates }) {
  ...
  const [custom, setCustom] = useState({ text: "", firmaAll: false, ortakAll: false, pin: false });
  const [tgGroups, setTgGroups] = useState([]);
  const [selGroups, setSelGroups] = useState({});
  const [selPersons, setSelPersons] = useState({});
  useEffect(() => { api("/api/tg/groups").then((r) => setTgGroups(r.groups || [])).catch(() => {}); }, []);
  ...
  const open = (title, items, pin) => setPreview({ title, items, pin: !!pin });
  const doSend = async (items, pin) => {
    ...
    setSending(true);
    try { const r = await api("/api/notify/send", { method: "POST", body: JSON.stringify({ items: msgs, pin: !!pin }) }); setResult({ sent: r.sent, failed: r.failed }); setPreview(null); }
  ...
  const sendCustom = () => {
    ...
    const addChat = (cid, label) => { const k = String(cid); if (!cid || seen.has(k)) return; seen.add(k); to.push({ chatId: cid, label }); };
    if (custom.firmaAll) firms.filter((f) => f.active).forEach((f) => chatsFor("firma", f.id).forEach((c) => addChat(c.chatId, f.name)));
    if (custom.ortakAll) shares.forEach((s) => chatsFor("karpayi", s.id).forEach((c) => addChat(c.chatId, s.name)));
    contacts.forEach((c, i) => { if (selPersons[c.chatId]) addChat(c.chatId, c.label || c.chatId); });
    tgGroups.forEach((g) => { if (selGroups[g.group_id]) addChat(g.group_id, "Grup: " + (g.title || g.group_id)); });
    const items = to.length ? [{ name: (custom.pin ? "📌 Sabitli · " : "") + to.length + " alıcı", to, msg: custom.text }] : [];
    open("Özel Mesaj Önizleme", items, custom.pin);
  };
  ...
  return (
    <div>
      ...
      <div className={card + " p-4 mb-4"}>
        ...
        <div className="text-xs text-slate-500 mb-3">Otomatik gönderimde <b>bir önceki günün</b> verisi gönderilir (gün sonu raporu). Aşağıdaki "Şimdi Çalıştır" ve elle gönderimler ise seçili tarihin <b>anlık</b> verisini kullanır.</div>
      ...
      <div className={card + " p-4 mb-4"}>
        ...
        <div className="mt-3 space-y-3">
          <div className="flex items-center gap-4 flex-wrap text-sm"><label className="flex items-center gap-2"><input type="checkbox" checked={custom.firmaAll} onChange={(e) => setCustom({ ...custom, firmaAll: e.target.checked })} /> Tüm firma kişileri</label><label className="flex items-center gap-2"><input type="checkbox" checked={custom.ortakAll} onChange={(e) => setCustom({ ...custom, ortakAll: e.target.checked })} /> Tüm ortaklar</label></div>
          {tgGroups.length > 0 && (<div><div className="text-xs uppercase text-slate-500 mb-1">Gruplar (tek tek seç)</div><div className="flex flex-wrap gap-2">{tgGroups.map((g) => <Chip key={g.group_id} on={!!selGroups[g.group_id]} onClick={() => setSelGroups({ ...selGroups, [g.group_id]: !selGroups[g.group_id] })}>{g.title || g.group_id}</Chip>)}</div></div>)}
          {contacts.length > 0 && (<div><div className="text-xs uppercase text-slate-500 mb-1">Kişiler (tek tek seç)</div><div className="flex flex-wrap gap-2" style={{ maxHeight: 120, overflowY: "auto" }}>{contacts.map((c, i) => <Chip key={i} on={!!selPersons[c.chatId]} onClick={() => setSelPersons({ ...selPersons, [c.chatId]: !selPersons[c.chatId] })}>{c.label || c.chatId}</Chip>)}</div></div>)}
          <div className="flex items-center gap-4 flex-wrap"><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={custom.pin} onChange={(e) => setCustom({ ...custom, pin: e.target.checked })} /> 📌 Sabitle (pin)</label><button className={btnP} disabled={!custom.text.trim()} onClick={sendCustom}><Send size={13} /> Önizle / Gönder</button></div>
        </div>
      </div>
...
/* ============ Denetim / Mutabakat ============ */
function Denetim({ firms, tx, wd, transfers, adjusts, auditLog, firmBalance, addDeposit, editDeposit, delDeposit, pushAudit, setConfirm }) {
  const [dateModal, setDateModal] = useState(null);
  const [newDate, setNewDate] = useState(todayIso());
  const okDate = (dt) => { if (!dt || typeof dt !== "string" || !/^\d{4}-\d{2}-\d{2}$/.test(dt)) return false; const [y, m, d] = dt.split("-").map(Number); if (!y || !m || !d || m > 12 || d > 31) return false; if (y < 2024 || y > 2027) return false; return true; };
  const firmName = (id) => firms.find((f) => f.id === id)?.name || "?";

  // 1) Firma bazında mutabakat: kasa vs tüm işlem toplamı
  const perFirm = firms.map((f) => {
    const d = tx.filter((t) => t.firmId === f.id), w = wd.filter((x) => x.firmId === f.id);
    const kasa = firmBalance(f.id);
    const virman = r2((transfers || []).reduce((s2, t) => s2 + ((t.toFirmId === f.id ? t.amount : 0) - (t.fromFirmId === f.id ? t.amount : 0)), 0));
    const duzeltme = r2((adjusts || []).filter((a) => a.firmId === f.id).reduce((s2, a) => s2 + (a.amount || 0), 0));
    return { f, kasa, adet: d.length, cadet: w.length, netTop: r2(d.reduce((a, t) => a + (t.net || 0), 0)), cekTop: r2(w.reduce((a, x) => a + (x.gross || 0), 0)), virman, duzeltme };
  });

  // 2) Tarih anomalileri
  const badDateTx = tx.filter((t) => !okDate(t.date)).map((t) => ({ ...t, _kind: "tx" }))
    .concat(wd.filter((x) => !okDate(x.date)).map((x) => ({ ...x, _kind: "wd" })));

  // 3) OLASI kayıp işlem: audit'te "Yatırım eklendi" var ama o firmanın O GÜNE ait hiç canlı işlemi yok
  const parseAmt = (s) => { const m = String(s || "").match(/([\d.]+),\d{2}/); return m ? Number(m[1].replace(/\./g, "")) : null; };
  const auditLocalDate = (iso) => { try { return new Intl.DateTimeFormat("en-CA", { timeZone: "Europe/Istanbul", year: "numeric", month: "2-digit", day: "2-digit" }).format(new Date(iso)); } catch (e) { return null; } };
  const lostBlocks = useMemo(() => {
    const txByFD = {}; tx.forEach((t) => { const k = t.firmId + "|" + t.date; txByFD[k] = (txByFD[k] || 0) + 1; });
    const trashKeys = new Set(); (typeof trash !== "undefined" ? [] : []).forEach(() => {});
    const cand = {};
    auditLog.forEach((a) => {
      if (!String(a.action || "").includes("Yatırım eklendi")) return;
      const detail = String(a.detail || ""); const gross = parseAmt(detail);
      const fname = detail.split("·")[0].trim();
      const f = firms.find((x) => x.name === fname); if (!f || !gross) return;
      const date = auditLocalDate(a.at); if (!date) return;
      if ((txByFD[f.id + "|" + date] || 0) > 0) return; // o gün canlı işlem varsa güvenli değil, atla
      const bkey = f.id + "|" + date;
      const b = cand[bkey] = cand[bkey] || { firmId: f.id, fname: f.name, date, items: [] };
      b.items.push({ firmId: f.id, fname: f.name, gross, date });
    });
    return Object.values(cand).sort((a, b) => (a.date < b.date ? 1 : -1));
  }, [auditLog, tx, firms]);
  const lostTotal = lostBlocks.reduce((a, b) => a + b.items.length, 0);

  const negFirms = perFirm.filter((r) => r.kasa < 0);
  const recover = (r) => addDeposit({ firmId: r.firmId, amount: r.gross, date: r.date, note: "Denetim kurtarma" });

  return (
    <div>
      <Header title="Denetim / Mutabakat" sub="Kasa doğrulama · anomali tespiti · kayıp işlem kurtarma" />

      {lostBlocks.length > 0 && (
        <div className="bg-red-500/10 border border-red-500/40 rounded-xl p-4 mb-5">
          <div className="flex items-center gap-2 font-bold text-red-300 mb-2"><AlertTriangle size={17} /> Olası kayıp işlem: {lostTotal} ({lostBlocks.length} firma-gün)</div>
          <div className="text-xs text-slate-400 mb-3">Bu işlemler geçmişte eklenmiş (işlem kaydında var) ama o firmanın o güne ait <b>hiç canlı işlemi yok</b> — büyük ihtimalle eski senkron sürümünde silinmiş. <b className="text-amber-300">Önce kendi kayıtlarınla (Telegram grubu vb.) doğrula</b>, sonra gerçekten eksik olan günü/işlemi geri yükle. (Düzenlenmiş/taşınmış olabilecekleri geri yükleme.)</div>
          <div className="space-y-3" style={{ maxHeight: 400, overflowY: "auto" }}>{lostBlocks.map((b) => { const tot = b.items.reduce((a, i) => a + i.gross, 0); return (
            <div key={b.firmId + b.date} className="border border-slate-800 rounded-lg overflow-hidden">
              <div className="flex items-center justify-between px-3 py-2 bg-slate-950/60"><div><b>{b.fname}</b> · {trDate(b.date)} <span className="text-xs text-slate-500">· {b.items.length} işlem · {fmt(tot)}</span></div><button className={btnG} onClick={() => setConfirm({ msg: `${b.fname} · ${trDate(b.date)} — ${b.items.length} işlem (${fmt(tot)}) geri yüklenecek. Bu günün gerçekten kaybolduğundan emin misin?`, onYes: () => b.items.forEach((i) => addDeposit({ firmId: i.firmId, amount: i.gross, date: i.date, note: "Denetim kurtarma" })) })}>Bu Günü Geri Yükle</button></div>
              <table className="w-full text-sm"><tbody>{b.items.map((i, ix) => (<tr key={ix} className="border-t border-slate-800/60"><td className="px-3 py-1.5 text-slate-400 text-xs">{trDate(i.date)}</td><td className="px-3 py-1.5 text-right font-semibold text-emerald-400">{fmt(i.gross)}</td><td className="px-3 py-1.5 text-center" style={{ width: 110 }}><button className="text-blue-400 hover:text-blue-300 text-xs font-semibold" onClick={() => addDeposit({ firmId: i.firmId, amount: i.gross, date: i.date, note: "Denetim kurtarma" })}>Geri Yükle</button></td></tr>))}</tbody></table>
            </div>); })}
          </div>
        </div>
      )}
      ...
      {badDateTx.length > 0 && (
        <div className="bg-amber-500/10 border border-amber-500/40 rounded-xl p-4 mb-5">
          <div className="flex items-center gap-2 font-bold text-amber-300 mb-2"><AlertTriangle size={17} /> Tarih anomalisi: {badDateTx.length}</div>
          <div className="text-xs text-slate-400 mb-3">Bu işlemlerin tarihi boş/hatalı — kasada sayılır ama tarih filtreli listelerde görünmez. Tarihi düzelt.</div>
          <div className="border border-slate-800 rounded-lg overflow-hidden">
            <table className="w-full text-sm"><thead><tr className="text-xs uppercase text-slate-500 bg-slate-950/60"><th className="text-left px-4 py-2.5">Tür</th><th className="text-left px-4 py-2.5">Firma</th><th className="text-left px-4 py-2.5">Tarih</th><th className="text-right px-4 py-2.5">Tutar</th><th className="text-center px-4 py-2.5">Düzelt</th></tr></thead>
            <tbody>{badDateTx.map((t) => (<tr key={t._kind + t.id} className="border-t border-slate-800"><td className="px-4 py-2.5"><Badge tone={t._kind === "wd" ? "red" : "blue"}>{t._kind === "wd" ? "Çekim" : "Yatırım"}</Badge></td><td className="px-4 py-2.5 font-semibold">{firmName(t.firmId)}</td><td className="px-4 py-2.5 text-red-300 font-mono text-xs">{String(t.date) || "(boş)"}</td><td className="px-4 py-2.5 text-right">{fmt(t._kind === "wd" ? t.net : t.gross)}</td><td className="px-4 py-2.5 text-center">{t._kind === "tx" ? <button className="text-blue-400 hover:text-blue-300" onClick={() => { setDateModal(t); setNewDate(todayIso()); }}><Edit2 size={15} /></button> : <span className="text-xs text-slate-500">çekimden düzelt</span>}</td></tr>))}</tbody></table>
          </div>
        </div>
      )}

      {lostBlocks.length === 0 && badDateTx.length === 0 && (
        <div className="bg-emerald-500/10 border border-emerald-500/40 rounded-xl px-4 py-3 mb-5 text-sm text-emerald-300 flex items-center gap-2"><Check size={17} /> Kayıp işlem ve tarih anomalisi bulunamadı. Veri tutarlı. 🎉</div>
      )}

      <div className={card + " overflow-hidden mb-5"}>
        <div className="px-4 py-3 border-b border-slate-800 font-bold flex items-center justify-between"><span>Firma Bazında Mutabakat</span>{negFirms.length > 0 && <Badge tone="red">{negFirms.length} firma negatif kasa</Badge>}</div>
        <table className="w-full text-sm"><thead><tr className="text-xs uppercase text-slate-500 bg-slate-950/40"><th className="text-left px-4 py-3">Firma</th><th className="text-right px-4 py-3">Yat. Adet</th><th className="text-right px-4 py-3">Net Toplam</th><th className="text-right px-4 py-3">Çekim Toplam</th><th className="text-right px-4 py-3">Virman</th><th className="text-right px-4 py-3">Düzeltme</th><th className="text-right px-4 py-3">Kasa</th><th className="text-center px-4 py-3">Durum</th></tr></thead>
        <tbody>{perFirm.map((r) => (<tr key={r.f.id} className="border-t border-slate-800 hover:bg-slate-800/40"><td className="px-4 py-3 font-semibold">{r.f.name}{!r.f.active && <span className="text-xs text-slate-500"> (pasif)</span>}</td><td className="px-4 py-3 text-right">{r.adet}</td><td className="px-4 py-3 text-right text-emerald-400">{fmt(r.netTop)}</td><td className="px-4 py-3 text-right text-red-400">{fmt(r.cekTop)}</td><td className={"px-4 py-3 text-right " + (r.virman >= 0 ? "text-sky-400" : "text-orange-400")}>{r.virman >= 0 ? "+" : ""}{fmt(r.virman)}</td><td className={"px-4 py-3 text-right " + (r.duzeltme >= 0 ? "text-sky-400" : "text-orange-400")}>{r.duzeltme ? (r.duzeltme > 0 ? "+" : "") + fmt(r.duzeltme) : "-"}</td><td className={"px-4 py-3 text-right font-bold " + (r.kasa < 0 ? "text-red-400" : "text-slate-100")}>{fmt(r.kasa)}</td><td className="px-4 py-3 text-center">{r.kasa < 0 ? <Badge tone="red">negatif</Badge> : <Badge tone="green">normal</Badge>}</td></tr>))}</tbody></table>
      </div>

      {dateModal && (<Modal title="Tarih Düzelt" onClose={() => setDateModal(null)} footer={<><button className={btnG} onClick={() => setDateModal(null)}>İptal</button><button className={btnP} onClick={() => { editDeposit(dateModal.id, { firmId: dateModal.firmId, amount: dateModal.gross, date: newDate, note: dateModal.note || "" }); setDateModal(null); }}>Kaydet</button></>}><div className="text-sm mb-3">{firmName(dateModal.firmId)} · {fmt(dateModal.gross)}</div><Field label="Yeni Tarih"><input type="date" className={inputCls} value={newDate} onChange={(e) => setNewDate(e.target.value)} /></Field></Modal>)}
    </div>
  );
}

/* ============ Raporlar ============ */
function Rapor({ firms, tx, wd, shares, shareTx, expenses, adjusts, transfers, firmBalance }) {
  ...
  const balBefore = (fid, cut) => r2(tx.filter((t) => t.firmId === fid && t.date < cut).reduce((a, t) => a + t.net, 0) - wd.filter((w) => w.firmId === fid && w.date < cut).reduce((a, w) => a + w.gross, 0) + (adjusts || []).filter((a) => a.firmId === fid && a.date < cut).reduce((s2, x) => s2 + (x.amount || 0), 0) + (transfers || []).filter((t) => t.date < cut).reduce((s2, t) => s2 + ((t.toFirmId === fid ? t.amount : 0) - (t.fromFirmId === fid ? t.amount : 0)), 0));
  const devreden = perFirm.map((r) => { const f = r.f; const open = balBefore(f.id, rg.from); const donemNet = r2(dep.filter((t) => t.firmId === f.id).reduce((a, t) => a + t.net, 0) - wdr.filter((w) => w.firmId === f.id).reduce((a, w) => a + w.gross, 0) + (adjusts || []).filter((a) => a.firmId === f.id && inRange(a.date, rg)).reduce((s2, x) => s2 + (x.amount || 0), 0) + (transfers || []).filter((t) => inRange(t.date, rg)).reduce((s2, t) => s2 + ((t.toFirmId === f.id ? t.amount : 0) - (t.fromFirmId === f.id ? t.amount : 0)), 0)); return { f, open, yat: r.yat, cek: r.cek, kom: r.kom, donemNet, kalan: r2(open + donemNet) }; });
  const dvTot = devreden.reduce((a, r) => ({ open: a.open + r.open, yat: a.yat + r.yat, cek: a.cek + r.cek, kom: a.kom + r.kom, donemNet: a.donemNet + r.donemNet, kalan: a.kalan + r.kalan }), { open: 0, yat: 0, cek: 0, kom: 0, donemNet: 0, kalan: 0 });
  ...
  return (
    <div>
      ...
      <div className={card + " overflow-hidden mb-4"}>
        <div className="px-4 py-3 border-b border-slate-800 font-bold flex items-center gap-2"><Repeat size={15} className="text-blue-400" /> Devreden Kasa <span className="text-xs text-slate-500 font-normal">· {rg.label}</span></div>
        <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="text-xs uppercase text-slate-500 bg-slate-950/40"><th className="text-left px-4 py-3">Firma</th><th className="text-right px-4 py-3">Devreden</th><th className="text-right px-4 py-3">Yatırım</th><th className="text-right px-4 py-3">Çekim</th><th className="text-right px-4 py-3">Komisyon</th><th className="text-right px-4 py-3">Dönem Net</th><th className="text-right px-4 py-3">Kalan Kasa</th></tr></thead><tbody>
          {devreden.map((r) => (<tr key={r.f.id} className="border-t border-slate-800"><td className="px-4 py-2.5">{r.f.name}</td><td className="px-4 py-2.5 text-right text-slate-400">{fmt(r.open)}</td><td className="px-4 py-2.5 text-right text-emerald-400">{fmt(r.yat)}</td><td className="px-4 py-2.5 text-right text-red-400">{fmt(r.cek)}</td><td className="px-4 py-2.5 text-right text-amber-400">{fmt(r.kom)}</td><td className={"px-4 py-2.5 text-right " + (r.donemNet >= 0 ? "text-emerald-400" : "text-red-400")}>{fmt(r.donemNet)}</td><td className="px-4 py-2.5 text-right font-bold">{fmt(r.kalan)}</td></tr>))}
        </tbody><tfoot><tr className="border-t-2 border-slate-700 font-bold bg-slate-950/40"><td className="px-4 py-3">TOPLAM</td><td className="px-4 py-3 text-right">{fmt(dvTot.open)}</td><td className="px-4 py-3 text-right text-emerald-400">{fmt(dvTot.yat)}</td><td className="px-4 py-3 text-right text-red-400">{fmt(dvTot.cek)}</td><td className="px-4 py-3 text-right text-amber-400">{fmt(dvTot.kom)}</td><td className="px-4 py-3 text-right">{fmt(dvTot.donemNet)}</td><td className="px-4 py-3 text-right">{fmt(dvTot.kalan)}</td></tr></tfoot></table></div>
      </div>
...
/* ============ IT ============ */
function TgYonetim({ firms }) {
  const [groups, setGroups] = useState(null);
  const [log, setLog] = useState([]);
  const [form, setForm] = useState({ group_id: "", firm_id: "", active: true });
  const [busy, setBusy] = useState(false);
  const load = async () => { try { const g = await api("/api/tg/groups"); setGroups(g.groups || []); } catch (e) { setGroups([]); } try { const l = await api("/api/tg/log"); setLog(l.log || []); } catch (e) {} };
  useEffect(() => { load(); }, []);
  const firmName = (id) => firms.find((f) => f.id === id)?.name || "-";
  const save = async () => { if (!form.group_id || !form.firm_id) return; setBusy(true); try { await api("/api/tg/groups", { method: "POST", body: JSON.stringify({ group_id: form.group_id.trim(), firm_id: Number(form.firm_id), title: firmName(Number(form.firm_id)), active: form.active }) }); setForm({ group_id: "", firm_id: "", active: true }); await load(); } catch (e) {} setBusy(false); };
  const toggle = async (g) => { try { await api("/api/tg/groups", { method: "POST", body: JSON.stringify({ group_id: g.group_id, firm_id: g.firm_id, title: g.title, active: !g.active }) }); load(); } catch (e) {} };
  const del = async (gid) => { try { await api("/api/tg/groups/" + encodeURIComponent(gid), { method: "DELETE" }); load(); } catch (e) {} };
  return (
    <div className="space-y-4">
      <div className={card + " p-5"}>
        <div className="font-bold mb-1 flex items-center gap-2"><Send size={16} className="text-sky-400" /> Telegram Bot Yönetimi <Badge tone="purple">v2.0</Badge></div>
        <div className="text-xs text-slate-500 mb-4">Botu gruba ekleyip grupta <b>!info</b> yazın; çıkan grup ID'sini buraya girip firmaya bağlayın. Bağlı grupta <b>!5.000</b> gibi onaylar otomatik işlenir; yanlışsa onay mesajına <b>!iptal</b> yanıtı verin.</div>
        <div className="grid md:grid-cols-3 gap-3">
          <Field label="Grup ID"><input className={inputCls} placeholder="-1001234567890" value={form.group_id} onChange={(e) => setForm({ ...form, group_id: e.target.value })} /></Field>
          <Field label="Firma"><select className={inputCls} value={form.firm_id} onChange={(e) => setForm({ ...form, firm_id: e.target.value })}><option value="">Seçin</option>{firms.filter((f) => f.active).map((f) => <option key={f.id} value={f.id}>{f.name}</option>)}</select></Field>
          <div className="flex items-end"><button className={btnP + " w-full"} disabled={busy} onClick={save}>Kaydet / Bağla</button></div>
        </div>
      </div>
      <div className={card + " overflow-hidden"}>
        <div className="px-4 py-3 border-b border-slate-800 font-bold">Tanımlı Gruplar</div>
        <table className="w-full text-sm"><thead><tr className="text-xs uppercase text-slate-500 bg-slate-950/40"><th className="text-left px-4 py-3">Grup ID</th><th className="text-left px-4 py-3">Firma</th><th className="text-center px-4 py-3">Durum</th><th className="text-right px-4 py-3">Sil</th></tr></thead><tbody>
          {(groups || []).map((g) => (<tr key={g.group_id} className="border-t border-slate-800"><td className="px-4 py-2.5 font-mono text-xs">{g.group_id}</td><td className="px-4 py-2.5">{firmName(g.firm_id)}</td><td className="px-4 py-2.5 text-center"><button onClick={() => toggle(g)}>{g.active ? <Badge tone="green">aktif</Badge> : <Badge tone="slate">pasif</Badge>}</button></td><td className="px-4 py-2.5 text-right"><button className="text-red-400 hover:text-red-300" onClick={() => del(g.group_id)}><Trash2 size={15} /></button></td></tr>))}
          {groups && groups.length === 0 && <tr><td colSpan={4} className="px-4 py-8 text-center text-slate-500">Henüz grup tanımlı değil.</td></tr>}
          {!groups && <tr><td colSpan={4} className="px-4 py-8 text-center text-slate-500">Yükleniyor…</td></tr>}
        </tbody></table>
      </div>
      <div className={card + " overflow-hidden"}>
        <div className="px-4 py-3 border-b border-slate-800 font-bold flex items-center justify-between"><span>Bot İşlem Kaydı</span><button className={btnG} onClick={load}>Yenile</button></div>
        <table className="w-full text-sm"><thead><tr className="text-xs uppercase text-slate-500 bg-slate-950/40"><th className="text-left px-4 py-3">Zaman</th><th className="text-left px-4 py-3">Firma</th><th className="text-left px-4 py-3">İşlem</th><th className="text-right px-4 py-3">Tutar</th></tr></thead><tbody>
          {log.map((l) => (<tr key={l.id} className="border-t border-slate-800"><td className="px-4 py-2 text-xs text-slate-400">{trDT(l.at)}</td><td className="px-4 py-2">{firmName(l.firm_id)}</td><td className="px-4 py-2">{l.action === "iptal" ? <Badge tone="red">iptal</Badge> : <Badge tone="green">yatırım</Badge>}</td><td className="px-4 py-2 text-right">{l.detail}</td></tr>))}
          {log.length === 0 && <tr><td colSpan={4} className="px-4 py-8 text-center text-slate-500">Kayıt yok.</td></tr>}
        </tbody></table>
      </div>
    </div>
  );
...
function IT({ auditLog, loginLog, errorLog, setAuditLog, setLoginLog, setErrorLog, pushError, trash, restoreTrash, purgeTrash, sessions, closeSession, ipAllow, setIpAllow, sIp, autoBackup, setAutoBackup, snapshot, applySnapshot, bannedIps, setBannedIps }) {  const [sub, setSub] = useState("islem");
  const [srvErrors, setSrvErrors] = useState(null);
  const [errBusy, setErrBusy] = useState(false);
  const [errDetail, setErrDetail] = useState(null);
  const loadErrors = async () => { setErrBusy(true); try { const r = await api("/api/errors"); setSrvErrors(r.errors || []); } catch (e) { setSrvErrors([]); } setErrBusy(false); };
  useEffect(() => { if (sub === "hata") loadErrors(); }, [sub]);
  ...
  return (
    <div>
      ...
      {sub === "hata" && (<div className={card + " overflow-hidden"}><div className="px-4 py-3 border-b border-slate-800 flex items-center justify-between flex-wrap gap-2"><span className="font-bold">Hata Logları <span className="text-xs text-slate-500 font-normal">· tüm kullanıcıların panellerinden, sunucuda kalıcı</span></span><div className="flex gap-2 gc-btnrow"><button className={btnG} onClick={loadErrors} disabled={errBusy}>{errBusy ? "..." : "Yenile"}</button><button className={btnG} onClick={() => { pushError("Örnek: ödeme servisi zaman aşımı", "at PaymentService.charge (server.js:142)", "test"); setTimeout(loadErrors, 800); }}>Örnek Hata</button><button className={btnD} onClick={async () => { try { await api("/api/errors", { method: "DELETE" }); } catch (e) {} setErrorLog([]); loadErrors(); }}>Temizle</button></div></div><table className="w-full text-sm"><thead><tr className="text-xs uppercase text-slate-500 bg-slate-950/40"><th className="text-left px-4 py-3">Zaman</th><th className="text-left px-4 py-3">Kullanıcı</th><th className="text-left px-4 py-3">Mesaj</th><th className="text-left px-4 py-3">Konum</th><th className="text-center px-4 py-3">Log</th></tr></thead><tbody>{(srvErrors || []).map((a) => (<tr key={a.id} className="border-t border-slate-800 hover:bg-slate-800/40"><td className="px-4 py-2.5 text-slate-400 text-xs whitespace-nowrap">{trDT(a.at)}</td><td className="px-4 py-2.5"><span className="font-semibold">{a.username || "-"}</span>{a.role && <span className="text-slate-500 text-xs"> · {a.role}</span>}</td><td className="px-4 py-2.5 text-red-300">{a.message}</td><td className="px-4 py-2.5 text-slate-500 font-mono text-xs">{(a.stack || "").split("\n")[0] || (a.ctx || "-")}</td><td className="px-4 py-2.5 text-center"><button className="text-blue-400 hover:text-blue-300" onClick={() => setErrDetail(a)}><Search size={14} /></button></td></tr>))}{srvErrors && srvErrors.length === 0 && <tr><td colSpan={5} className="px-4 py-10 text-center text-slate-500">Hata yok 🎉</td></tr>}{!srvErrors && <tr><td colSpan={5} className="px-4 py-10 text-center text-slate-500">Yükleniyor…</td></tr>}</tbody></table>
        {errDetail && <Modal title="Hata Detayı" onClose={() => setErrDetail(null)} wide footer={<button className={btnG} onClick={() => setErrDetail(null)}>Kapat</button>}><div className="text-xs text-slate-500 mb-1">Zaman</div><div className="mb-3">{trDT(errDetail.at)}</div><div className="text-xs text-slate-500 mb-1">Kullanıcı / Rol / IP</div><div className="mb-3">{errDetail.username || "-"} · {errDetail.role || "-"} · <span className="font-mono">{errDetail.ip || "-"}</span></div><div className="text-xs text-slate-500 mb-1">Kaynak</div><div className="mb-3">{errDetail.ctx || "-"}</div><div className="text-xs text-slate-500 mb-1">Mesaj</div><div className="text-red-300 mb-3">{errDetail.message}</div><div className="text-xs text-slate-500 mb-1">Tam Log</div><pre className="bg-slate-950 border border-slate-800 rounded-lg p-3 text-xs text-slate-300 whitespace-pre-wrap overflow-x-auto" style={{ maxHeight: 320 }}>{errDetail.stack || "(log yok)"}</pre></Modal>}
      </div>)}
